...
uint16_t Topics::getTopicId(MQTTSN_topicid* topicid)
{
	...
	while (p)
	{
		if (p->_topicName->compare(0,topicid->data.long_.len, topicid->data.long_.name) == 0)
		{
			return p->_topicId;
		}
		p = p->_next;
...
Topic* Topics::getTopic(MQTTSN_topicid* topicid)
{
	...
	while (p)
	{
		if (p->_topicName->compare(0,topicid->data.long_.len - 1, topicid->data.long_.name) == 0)
		{
			return p;
		}
		p = p->_next;
