...
void MQTTSNPublishHandler::handleRegister(Client* client, MQTTSNPacket* packet)
{
	...
	packet->getREGISTER(&topicid, &msgId, &topicName);
	string topic = string(topicName.lenstring.data, topicName.lenstring.len);
	topicid = client->getTopics()->add(&topic)->getTopicId();
	regAck->setREGACK(topicid, msgId, MQTTSN_RC_ACCEPTED);
	Event* ev = new Event();
